...
public class ClassOverview extends JPanel 
{
	...
	//Init JCombo Boxes
	JComboBox<String> semesterBox, classBox, testBox;

	//Init Btns
	CustomButton addSemester, removeSemester, addClass, removeClass, addTest, removeTest, saveQuestion;
	CustomButton questionFun[] = new CustomButton[9];
	...
	int WID, HEI, num = 1, btnHeight = 35;
	...
	public void addPanel1(int width, int height)
	{
		...
		classBox = new JComboBox<String>(opti);
		...
		classBox.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e)
			{
				...
				/* Adds items to the combo box using teh directory structure
		...
		addClass = new CustomButton("Add Class");
		addClass.setBounds(addSemester.getX(), 150, addSemester.getWidth(), btnHeight);
		addClass.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e)
			{
				...
				if (classInputByUser != null) 
				{
					...
					for(Course temp : LibraryController.getAllCourses()){
						classBox.addItem(temp.getName());
					}
		...
		removeClass = new CustomButton("Remove Class");
		removeClass.setBounds(removeSemester.getX(), 150, removeSemester.getWidth(), btnHeight);
		...
		addTest = new CustomButton("Add Test");
		addTest.setBounds(addSemester.getX(), 240, addSemester.getWidth(), btnHeight);
		...
		removeTest = new CustomButton("Remove Test");
		removeTest.setBounds(removeClass.getX(), 240, removeClass.getWidth(), btnHeight);
	...
	public void addPanel3(int width, int height)
	{
		...
		/*for (int i = 0; i < questionFun.length; i++) {
			questionFun[i] = new CustomButton();
		} */
		
		int btnHeightFun = (int)(btnHeight);

		questionFun[0] = new CustomButton("Add Question");
		questionFun[0].setBounds(width/4 - 85, height/5 + 5, width/4, btnHeightFun);
		questionFun[0].addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				close.setBounds(create.getWidth() - 105, 5, 100, 50);
				create.add(close);
				...
				saveQuestion = new CustomButton("Save");
				saveQuestion.setBounds(5, create.getPanelHeight() - 30, 100, 50);
		...
		questionFun[1] = new CustomButton("Create Final");
		questionFun[1].setBounds(questionFun[0].getX(), testInfo[0].getY() + btnHeight, width/4, btnHeightFun);
		...
		questionFun[2] = new CustomButton("View Questions");
		questionFun[2].setBounds(width/2 - (width/4)/2 + 2, questionFun[0].getY(), width/4, btnHeightFun);
		questionFun[2].addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e)
			{
				...
				CustomButton x = new CustomButton("Quit");
		...
		questionFun[3] = new CustomButton("Flash Cards");
		questionFun[3].setBounds(questionFun[2].getX(), questionFun[1].getY(), width/4, btnHeightFun);
		...
		questionFun[4] = new CustomButton("Study Jam!");
		questionFun[4].setBounds(questionFun[2].getX() + questionFun[0].getWidth() + 20, questionFun[0].getY(), width/4, btnHeightFun);
		...
		questionFun[5] = new CustomButton("Take Test!");
		questionFun[5].setBounds(questionFun[4].getX(), questionFun[1].getY(), width/4, btnHeightFun);
		//for the test screen:
		//Create curDirectory
		CustomButton close = new CustomButton("Quit");
		close.setBounds(getWidth() - 130, 10, 75, btnHeight);
		close.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e)
			{
	...
	public void semester(int width)
	{
		...
		addSemester = new CustomButton("Add Semester");
		...
		addSemester.setBounds(width - (width/2) - 5, 60, width/2, btnHeight);
		panels[0].add(addSemester);

		removeSemester = new CustomButton("Remove Semester");
		...
		removeSemester.setBounds(3, 60, width/2, btnHeight);
	...
	public void addPanel2(int width, int height)
	{
		...
		for (int i = 0; i < 4; i++) {
			...
			case 0:
				//top
				statsBorder[i].setBounds(52, 15, width - (int)(statsLabel.getWidth()/1.75), 1);
