...
public class StudyGame extends JPanel {
	...
	int width, height, questionsShown = 0, currentCount, score = 0, seconds = 10, answeredCorrect = 0, attempted = 0,
			totalAdded = 0;
	...
	JButton nextBtn;
	JComboBox<String> answers[] = new JComboBox[5];
	JTextArea missedQuestionsArea;
	JScrollPane missedQuestionsScroller;
	String master = "";

	public StudyGame(int w, int h, JPanel[] in) {
		// Set Vars
		width = w;
		height = h;
		main = in;
		 ...
		 * 
		 * 1) On start, there will be five questions to view. Beneath each
		 * question, there is a JComboBox. 2) The user must respond to the
		 * question with the correct answer from the comboBox as quickly as they
		 * can. 3) The catch? - They only have 10 seconds to answer each set or
		 * the game ends. The user gets 3 Seconds for each correct response So
		 * that they can continue.
		 * 
		 * In addition, each correct answer is worth 10 points where incorrect
		 * repsponses are -5. Not answering a question results in a simple -2.
		 * This is because the question will keep appearing anyways until the
		 * user gets it right or runs out of time.
		 * 
		 * 4) The game ends when each question has been correctly responded to
		 * at least 3 times. Or, time runs out. The score for each game is saved
		 * in a "master" file with the date earned.
		 * 
		 * Notes: The question minimum is 10. If there aren't ten questions yet,
		 * the user must add them This will be overridded anyways becuase each
		 * question is added twice.
		 * 
		 */
	...
	public void scan(int cou) {
		...
		if ((index > 0) && (!checked[cou])) {
			...
			if (selected.equalsIgnoreCase(correct)) {
				...
				// addQuestion(myQuestions, questionArea[cou].getText().trim(),
				// 1);
				...
				// addQuestion(missedQuestions,
				// questionArea[cou].getText().trim(), 1);
	...
	public void updateMaster(String question, String selected, String correctAnswer, boolean correct) {
		if (correct) {
			...
			// Check to see if the question already exists. If it does, do not
			// include it again in the master, just increase the count
	...
	public void buildLayout() {
		/*
		 * The layout will be comprised of the following: 1) five scrolling text
		 * areas. This is for each question. Each one will be only 3 rows deep,
		 * but will take up half of the screen Also, They will be set to start
		 * at the top of the view 2) There is a JComboBox with the possible
		 * answers to the right of the top of each scrollview 3) Respondingly,
		 * there is a JtextArea for each scrollview 4) there will be a label for
		...
		for (int i = 0; i < 5; i++) {
			...
			questionScroll[i] = new JScrollPane(questionArea[i], JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
			...
			questionTitle[i].setBounds(5, questionScroll[i].getY(), questionScroll[i].getX() - 5,
					questionScroll[i].getHeight());
			...
			// Add actionlistener to the JCombo. When the user selects one, it
			// becomes disabled (if not index 0).
			// Then, once disabled, it will check to see if the answer is
			// correct.
			...
			answers[i].setBounds(questionScroll[i].getX() + questionScroll[i].getWidth(), questionScroll[i].getY(),
					width - 10 - (questionScroll[i].getX() + questionScroll[i].getWidth()), 25);
			add(answers[i]);
			questionsShown++;
		}
		...
		// Add Score Label
		scoreLbl.setBounds(questionScroll[0].getX(), 5, width - (questionScroll[0].getX() * 2),
				questionArea[0].getY() - 5);
	...
	public void getInfo() {
		...
		for (Question temp : this.myQuestions) {
			System.out.println("Question: " + temp.toString());
		}
	}
	...
	public void finish() {
		...
		// Now that we have the score, we are going to update the list of scores
		// we have for that test. Note that this will be a contant list update
		// because we are just saving the scores here versus the entire test
		// like previously.
		...
		// We are going to create a finish screen. First, hide everything on the
		// panel
		int areaCounter = 0;
		for (JTextArea i : questionArea) {
			if (questionsShown < myQuestions.size()) {
		...
		missedQuestionsScroller = new JScrollPane(missedQuestionsArea, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		...
		// missedQuestionsScroller.setBorder(BorderFactory.createEmptyBorder(0,
		// 0, 0, 0));
		missedQuestionsScroller.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);

		// After the file was updated, we aer going to show the user what
		// questions they missed and how many times it was missed. We are going
		// to do this by checking the missedQuestions arraylist and determining
		// the count as well as the quantity of each question (using a method).

		JOptionPane.showMessageDialog(new JFrame("Final Score"),
